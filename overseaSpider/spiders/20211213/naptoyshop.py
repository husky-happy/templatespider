...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> naptoyshop
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['naptoyshop.com']
    start_urls = ['https://www.naptoyshop.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath('//a[@class="_27Yy0"]/@href').getall()
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//a[@class="_3mKI1"]/@href').getall()
    ...
    def parse_detail(self, response):
        ...
        price = response.xpath('//div[@class="_26qxh"]/span[1]/text()').get()
        ...
        name = response.xpath('//div[@class="fggS- cell"]/h1/text()').get()
        items["name"] = name

        cat_list = response.xpath('//div[@data-hook="breadcrumbs"]/a/text()').getall()
        if cat_list:
        ...
        images_list = response.xpath('//div[@class="_3j9OG media-wrapper-hook V-iTp"]//@href').getall()
        ...
        detection_main(items=items, website=website, num=10, skulist=False, skulist_attributes=True)
        # print(items)
        yield items
