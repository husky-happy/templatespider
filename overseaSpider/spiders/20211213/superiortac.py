...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> superiortac
@IDE    ：PyCharm
@Author ：Mr. Husky
...
website = 'superiortac'
website_url = 'https://shop.superiortac.com'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['shop.superiortac.com']
    start_urls = ['https://shop.superiortac.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath("//ul[@id='menubar1']/li/a/@href").getall()
        category_cate = response.xpath("//ul[@id='menubar1']/li/a/text()").getall()
    ...
    def parse_list(self, response):
        ...
        detail_url_list = response.xpath("//table[@class='productTable']//tr/td[@class='imageRow']/a/@href").getall()
        for detail_url in detail_url_list:
            detail_url = website_url + detail_url
            yield scrapy.Request(url=detail_url, callback=self.parse_detail, meta={"cat":cate})
        next_page_url = response.xpath("//a[@class='page' and text()='»']/@href").get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        cate = response.meta.get("cat")
        price = response.xpath("//span[@id='listPrice']/text()").get()
        ...
        cat_list = ['Our Products',cate,name]
        if cat_list:
            cat_list = [cat.strip() for cat in cat_list if cat.strip()]
            items["cat"] = cat_list[-1]
            items["detail_cat"] = '/'.join(cat_list)

        description = response.xpath("//div[contains(@class,'product-description')]").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = 'superiortac.com'
        ...
        images_list = response.xpath("//div[@class='filmstrip-thumbnails']//tr/td/a/@onclick").getall()
        for i in range(len(images_list)):
            images_list[i]=re.search(r"\('(.*?)','",images_list[i]).group(1)
        if not images_list:
            images_list = response.xpath("//div[@class='product-image']/img/@src").getall()
        for i in range(len(images_list)):
