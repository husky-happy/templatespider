...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> superchips
@IDE    ：PyCharm
@Author ：Mr. Husky
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['superchips.com']
    start_urls = ['https://superchips.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://superchips.com/shop/']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_brand)

    def parse_brand(self, response):
        category_urls = response.xpath("//li[@class='category--single l-col-4--sm']/a/@href").getall()
        category_cate = response.xpath("//li[@class='category--single l-col-4--sm']/a/span/text()").getall()
        for c in range(len(category_urls)):
            if category_cate[c]!="VIEW ALL":
    ...
    def parse_list(self, response):
        ...
        detail_url_list = response.xpath("//li[@class='category--product item']/div/a/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        cate = response.meta.get("cat")
        price_new = response.xpath("//span[@class='regular-price']/span/text()").get()
        price_new = self.price_fliter(price_new)
        price_old = response.xpath("//div[@class='retail--price']/span/text()").get()
        if price_old:
        ...
        description = response.xpath("//section[@id='introduction']").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = 'superchips.com'
        ...
        images_list = response.xpath("//ul[@id='js-product-img-slider']/li/img/@src").getall()
        for i in images_list:
            if i.find("placeholder")!=-1:
                return
        items["images"] = images_list
        items["brand"] = ''

        opt_name = response.xpath("//div[@id='product-options-wrapper']//label/text()").getall()
        if not opt_name:
            items["sku_list"] = []
        else:
            opt_value = []
            ...
            for i in range(opt_length):
                value_temp = response.xpath("//div[@id='product-options-wrapper']//select/option[@price]/text()").getall()
                if value_temp:
