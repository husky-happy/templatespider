...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['funtoysland.com']
    start_urls = ['https://funtoysland.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://funtoysland.com/product/']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//div[@class='product-item']/span/a/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        images_list = response.xpath("//div[@id='productSlider']/a/@href").getall()
        for i in range(len(images_list)):
            if images_list[i].find("https:")==-1:
                images_list[i] = "https:" + images_list[i]
        items["images"] = images_list

        if sku_len<=1 or skuAttr =='':
            items["sku_list"] = []
        else:
            sku_list = list()
