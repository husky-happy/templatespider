...
website = 'tikamoon'
website_url = 'https://www.tikamoon.co.uk'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['tikamoon.co.uk']
    start_urls = ['https://www.tikamoon.co.uk/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath("//div[@class='navigation-onglet']/ol/li[@class='fm parent']/ul/li/a/@href").getall()
    ...
    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//div[contains(@class,'productBox')]/a/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        price_old = response.xpath("//p[@class='price']/span/span[1]/text()").get()
        price_new = response.xpath("//p[@class='price']/span/text()[1]").get()
        ...
        cat_list = response.xpath("//ul[@class='breadcrumb']/li/a/span/text()").getall()
        if cat_list:
            cat_list = [cat.strip() for cat in cat_list if cat.strip()]
            items["cat"] = cat_list[-1]
            items["detail_cat"] = '/'.join(cat_list)

        description = response.xpath("//div[@class='panel-desc']/div").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = website

        images_list = response.xpath('//picture[@data-original]/@data-original').getall()
        items["images"] = images_list


        items["sku_list"] = []
        items["brand"]='Tikamoon'
