...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> 221creations
@IDE    ：PyCharm
@Author ：Mr. Husky
...
website = '221creations'
website_utl = 'https://www.221creations.com'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['221creations.com']
    start_urls = ['https://www.221creations.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.221creations.com/onlineshop.html']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list_tmp)

    def parse_list_tmp(self, response):
        category_urls = response.xpath("//div[@class='product-grid__item']/a/@href").getall()
        for category_url in category_urls:
    ...
    def parse_list(self, response):
        ...
        for detail_url in detail_url_list:
            detail_url = website_utl + detail_url
    ...
    def parse_detail(self, response):
        ...
        name = response.xpath("//h2[@id='wsite-com-product-title']/text()").get()
        name = self.filter_text(name)
        items["name"] = name

        cat_list = response.xpath("//ul[@id='wsite-com-breadcrumbs']/li//span[@class='wsite-com-link-text']/text()").getall()
        if cat_list:
            for c in range(len(cat_list)):
                cat_list[c] = cat_list[c].replace('\n','')
                cat_list[c] = cat_list[c].strip()
            items["cat"] = cat_list[-1]
            items["detail_cat"] = '/'.join(cat_list)

        description = response.xpath("//div[@itemprop='description']").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = '221creations.com'
        ...
        for i in range(len(images_list)):
            images_list[i] = website_utl + images_list[i]
        items["images"] = images_list
        items["brand"] = ''

        opt_name = response.xpath("//div[@class='wsite-com-product-option-groups']/div/label/b/text()").getall()
        if not opt_name:
            items["sku_list"] = []
        else:
            opt_value = []
            ...
            for i in range(opt_length):
                value_temp = response.xpath("//div[@class='wsite-com-product-option-groups']/div["+str(i+1)+"]/select/option[@value!='']/text()").getall()
