...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> pippenhillrooms
@IDE    ：PyCharm
@Author ：Mr. Husky
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['pippenhillrooms.com']
    start_urls = ['https://pippenhillrooms.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://pippenhillrooms.com/brand-new/',
                         'https://pippenhillrooms.com/newborn/',
                         'https://pippenhillrooms.com/accessories/',
                         'https://pippenhillrooms.com/apparel/',
                         'https://pippenhillrooms.com/gear/']
    ...
    def parse_detail(self, response):
        ...
        price = response.xpath("//div[@class='productView-price']//span[@class='price price--withTax price--main']/text()").get()
        ...
        items["source"] = 'pippenhillrooms.com'
        ...
        images_list = response.xpath("//ul[@class='productView-imageCarousel-main']/li/a/@href").getall()
        items["images"] = images_list
        brand = response.xpath("//h5[@itemprop='brand']/a/span/text()").get()
        if brand:
            items["brand"] = brand
        else:
            items["brand"] = ''

        opt_name = response.xpath("//div[@class='form-field']/select").getall()
        if not opt_name:
            items["sku_list"] = []
        else:
            opt_name = ['size']
            ...
            for i in range(opt_length):
                value_temp = response.xpath("//div[@class='form-field']/select/option[@data-product-attribute-value]/text()").getall()
                if value_temp:
