...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> bestvetcare
@IDE    ：PyCharm
@Author ：Mr. Husky
...
website = 'bestvetcare'
website_url = 'https://www.bestvetcare.com'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['bestvetcare.com']
    start_urls = ['https://www.bestvetcare.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.bestvetcare.com/dogs-supplies-1.aspx',
                         'https://www.bestvetcare.com/cats-supplies-2.aspx',
                         'https://www.bestvetcare.com/supplements-supplies-103.aspx',
                         'https://www.bestvetcare.com/homeopathic-supplies-106.aspx']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//div[@class='pro-supplies-col']/div[@class='product-img']/a/@href").getall()
    ...
    def parse_list_1(self, response):
        detail_url_list = response.xpath("//div[@class='clearfix prodlanding-box']/div[@class='landing-small-img']/a/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        cat_list = response.xpath("//div[@class='page-type-box']/ul/li/a/text()").getall()
        if cat_list:
            cat_list = [cat.strip() for cat in cat_list if cat.strip()]
            items["cat"] = cat_list[-1]
            items["detail_cat"] = '/'.join(cat_list)

        description = response.xpath("//div[@id='information1']").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = 'bestvetcare.com'
        ...
        images_list = response.xpath("//div[@class='Product_Bgimage']/img/@src").getall()
