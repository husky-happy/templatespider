...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> heavenlystarjewel
@IDE    ：PyCharm
@Author ：Mr. Husky
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['heavenlystarjewel.com']
    start_urls = ['https://heavenlystarjewel.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://heavenlystarjewel.com/occasional-tables/',
                         'https://heavenlystarjewel.com/readings-1/',
                         'https://heavenlystarjewel.com/jewelry/']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//ul[contains(@class,'ProductList')]/li/div[@class='ProductImage QuickView']/a/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        cat_list = response.xpath("//div[@id='ProductBreadcrumb']/ul[last()]/li//text()").getall()
        if cat_list:
        ...
        items["source"] = 'heavenlystarjewel.com'
        ...
        images_list_tmp = response.xpath("//div[@class='ProductTinyImageList']/ul/li//a/@rel").getall()
        images_list = []
        for i in images_list_tmp:
            img_json = json.loads(i)
            images_list.append(img_json["largeimage"])
        items["images"] = images_list
        brand = response.xpath("//div[@itemprop='brand']//span/text()").get()
        if brand:
            items["brand"] = brand
        else:
            items["brand"] = ''
