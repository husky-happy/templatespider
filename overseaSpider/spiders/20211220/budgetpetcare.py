...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> budgetpetcare
@IDE    ：PyCharm
@Author ：Mr. Husky
...
website = 'budgetpetcare'
website_url = 'https://www.budgetpetcare.com'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['budgetpetcare.com']
    start_urls = ['https://www.budgetpetcare.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.budgetpetcare.com/dog-supplies-1.aspx',
                         'https://www.budgetpetcare.com/cat-supplies-2.aspx',
                         'https://www.budgetpetcare.com/horse-supplies-104.aspx',
                         'https://www.budgetpetcare.com/bird-supplies-110.aspx',
                         'https://www.budgetpetcare.com/homeopathic-supplies-102.aspx',
                         'https://www.budgetpetcare.com/supplements-supplies-108.aspx',
                         'https://www.budgetpetcare.com/hygiene-supplies-109.aspx']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//div[@class='Home_ProBoxDiv']/div[@class='Home_Proimg']/a/@href").getall()
        for detail_url in detail_url_list:
            detail_url = website_url + detail_url
            yield scrapy.Request(url=detail_url, callback=self.parse_list_1, dont_filter=True)

    def parse_list_1(self, response):
        detail_url_list = response.xpath("//div[@class='Product_tableDiv']/div/a/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        price = response.xpath("//span[@id='PackSellingPrice']/text()").get()
        ...
        cat_list = response.xpath("//div[@class='breadcrumb']/ul/li//text()").getall()
        if cat_list:
            cat_list = [cat.strip() for cat in cat_list if cat.strip()]
            items["cat"] = cat_list[-1]
            items["detail_cat"] = '/'.join(cat_list)

        description = response.xpath("//div[@class='tab-paragraph']//div[@class='prod_overview']").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = 'budgetpetcare.com'
        ...
        images_list = response.xpath("//div[@id='imgpro']/img/@src").getall()
