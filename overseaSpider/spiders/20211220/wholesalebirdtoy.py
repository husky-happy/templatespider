...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> wholesalebirdtoy
@IDE    ：PyCharm
@Author ：Mr. Husky
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['wholesalebirdtoy.com']
    start_urls = ['https://wholesalebirdtoy.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://wholesalebirdtoy.com/bird-toy-parts/',
                         'https://wholesalebirdtoy.com/bird-toys/',
                         'https://wholesalebirdtoy.com/cups-for-cages/',
                         'https://wholesalebirdtoy.com/bird-cages-and-play-stands/cups-for-cages/',
                         'https://wholesalebirdtoy.com/health-care/',
                         'https://wholesalebirdtoy.com/huge/',
                         'https://wholesalebirdtoy.com/swings/',
                         'https://wholesalebirdtoy.com/weekly-sale-items/',
                         'https://wholesalebirdtoy.com/bulk-discounts/']
    ...
    def parse_list(self, response):
        ...
        # detail_url_list = ['https://wholesalebirdtoy.com/3279-natural-sterilized-hair-nesting-material/',
        #                    'https://wholesalebirdtoy.com/rainforest-mist-bath-spray-for-small-birds-8oz/',
        #                    'https://wholesalebirdtoy.com/bonka-happy-hut-tent-small/']
        detail_url_list.append('https://wholesalebirdtoy.com/bonka-happy-hut-tent-small/')
    ...
    def parse_detail(self, response):
        ...
        price_new_list = price_new.split("-")
        price_new = price_new_list[0]
        price_new = self.price_fliter(price_new)
        price_old = response.xpath("//div[@class='productView-price']//div[not(@style)]/span[@class='price price--non-sale']/text()").get()
        if price_old:
        ...
        cat_list = response.xpath("//ul[@class='breadcrumbs']/li/a/span/text()").getall()
        ...
        items["source"] = 'wholesalebirdtoy.com'
        ...
        brand = response.xpath("//h2/a/span[@itemprop='name']/text()").get()
        if brand:
            items["brand"] = brand
        else:
            items["brand"] = ''
        ...
        if not opt_name:
            ...
            # print(attrs_list)
            headers = {
                'authority': 'wholesalebirdtoy.com',
                'sec-ch-ua': '" Not A;Brand";v="99", "Chromium";v="96", "Google Chrome";v="96"',
                'stencil-config': '{}',
                'x-xsrf-token': '5e5b793c15cc37f97904822e000cab2f68e86f2a0a6930b3cb0d5b49a6c3e137, 5e5b793c15cc37f97904822e000cab2f68e86f2a0a6930b3cb0d5b49a6c3e137',
                'sec-ch-ua-mobile': '?0',
                ...
                'origin': 'https://wholesalebirdtoy.com',
                'sec-fetch-site': 'same-origin',
                'sec-fetch-mode': 'cors',
                'sec-fetch-dest': 'empty',
                'accept-language': 'zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7,an;q=0.6',
                'cookie': 'SHOP_SESSION_TOKEN=lguonsk398181df54m1njsg91m; fornax_anonymousId=4f6a95c1-9f29-431d-b919-6adc3254c992; XSRF-TOKEN=5e5b793c15cc37f97904822e000cab2f68e86f2a0a6930b3cb0d5b49a6c3e137; STORE_VISITOR=1; lastVisitedCategory=20; Shopper-Pref=72EC2E30F80F6E98520BD95C679CD790674BCDC0-1640591865738-x%7B%22cur%22%3A%22USD%22%7D; __atuvc=18%7C51; __atuvs=61c0340e87b0ccb9007',
            }
            sku_list = list()
            for attrs in attrs_list:
                ...
                response = requests.post('https://wholesalebirdtoy.com/remote/v1/product-attributes/'+product_id, headers=headers, data=data)
