...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> beyondhealth
@IDE    ：PyCharm
@Author ：Mr. Husky
@Date   ：2022/1/5 9:50
@Desc   ：
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['beyondhealth.com']
    start_urls = ['https://beyondhealth.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath("//ul[@class='navPage-subMenu-list']/li/a/@href").getall()
        category_cate = response.xpath("//ul[@class='navPage-subMenu-list']/li/a/text()").getall()
    ...
    def parse_detail(self, response):
        ...
        cat_list = ["Home","Shop",cate,name]
        if cat_list:
            cat_list = [cat.strip() for cat in cat_list if cat.strip()]
            items["cat"] = cat_list[-1]
            items["detail_cat"] = '/'.join(cat_list)

        description = response.xpath("//div[@class='co_short_descr']").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = 'beyondhealth.com'
        ...
        images_list = response.xpath("//div[@class='productCarousel-slide js-product-slide_t productView-thumbnail']/img/@src").getall()
        for i in range(len(images_list)):
            images_list[i] = images_list[i].replace("100x100","1280x1280")
