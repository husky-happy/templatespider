...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> sharmusic
@IDE    ：PyCharm
@Author ：Mr. Husky
...
website = 'sharmusic'
website_url = 'https://www.sharmusic.com'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['sharmusic.com']
    start_urls = ['https://www.sharmusic.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.sharmusic.com/Strings/',
                         'https://www.sharmusic.com/Sheet-Music/',
                         'https://www.sharmusic.com/Suzuki/',
                         'https://www.sharmusic.com/Instruments/',
                         'https://www.sharmusic.com/Bows/',
                         'https://www.sharmusic.com/Cases-Bags/',
                         'https://www.sharmusic.com/Digital/']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//ul[@class='list-product clearfix']/li/form/a[@data-edpno]/@href").getall()
        for detail_url in detail_url_list:
            detail_url = website_url + detail_url
            yield scrapy.Request(url=detail_url, callback=self.parse_detail)
        next_page_url = response.xpath("//a[@class='next-page update-search-results']/@href").get()
        if next_page_url:
            next_page_url = response.url.split("?")[0]+next_page_url
    ...
    def parse_detail(self, response):
        ...
        # if response.url == 'https://www.sharmusic.com/Cases-Bags/Bass/Heritage-Bass-Bag-5-mm-Foam-3-4-Size.axd':
        #     return
        price_new = response.xpath("//p[@class='price sale']/span/text()").get()
        if price_new=='($1)':
            price_new = response.xpath("//p[@class='price']/span/text()").get()
            price_new =self.price_fliter(price_new)
            price_old = price_new
        else:
            price_new = self.price_fliter(price_new)
            price_old = response.xpath("//p[@class='price small-price']/span/text()").get()
        ...
        cat_list = response.xpath("//ul[@id='breadcrumbs']/li/a/span/text()").getall()
        cat_list.insert(0,'Home')
        if cat_list:
            cat_list = [cat.strip() for cat in cat_list if cat.strip()]
            items["cat"] = cat_list[-1]
            items["detail_cat"] = '/'.join(cat_list)

        description = response.xpath("//div[@id='panel-0']").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = 'sharmusic.com'
        ...
        images_list = response.xpath("//ul[@id='thumbnail-reservoir']/li/div/a[@data-imagefor]/@href").getall()
        if len(images_list)==0:
            images_list = response.xpath("//ul[@id='list-alt-image-repeater']/li/div/a[@data-imagefor]/@href").getall()
        ...
        opt_name = response.xpath("//div[@class='style-selector-container']/div/p/strong/text()").getall()
        if not opt_name:
            items["sku_list"] = []
        else:
            ...
            for i in range(opt_length):
                value_temp = response.xpath("//div[@class='style-selector-container']/div["+str(i+1)+"]/ul/li/input/@data-styleleveldescription").getall()
                if value_temp:
