...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> pettsboro
@IDE    ：PyCharm
@Author ：Mr. Husky
@Date   ：2021/12/27 17:08
@Desc   ：
=================================================='''
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['pettsboro.com']
    start_urls = ['https://pettsboro.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://pettsboro.com/product-category/raw-dog-food/',
                         'https://pettsboro.com/product-category/raw-cat-food/',
                         'https://pettsboro.com/product-category/bones/']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//ul[@class='products products-list row grid']/li/div/div[@class='products-thumb']//a[not(@rel) and not(@data-product_id)]/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        price_new = response.xpath("//div[@class='price']/span/text()").get()
        price_old = response.xpath("//div[@class='price']/del/span/text()").get()
        if price_old:
        ...
        cat_list = response.xpath("//div[@itemprop='breadcrumb']/a/text()").getall()
        ...
        images_list = response.xpath("//div[@id='image-thumbnail']/div/a/img/@data-zoom-image").getall()
